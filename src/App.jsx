...
function BootScreen({ text = "Đang tải..." }) {
  return (
    <div
      style={{
        minHeight: "100vh",
...
function KitchenBoard({ currentProfile, onLogout }) {
  ...
  const currentHubId = String(currentProfile?.hub_id || "").trim();
  const currentBranchName =
    String(currentProfile?.branch_name || "").trim() || "Chưa gán chi nhánh";
  const currentDisplayName =
    String(currentProfile?.full_name || currentProfile?.email || "").trim() || "Nhân viên";
              ...
              >
                <div style={{ minWidth: 220 }}>
                  ...
                  <div style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>
                    APP nhận đơn • Chi nhánh {currentBranchName}
                  </div>
                </div>
  ...
  useEffect(() => {
    ...
    async function initSession() {
      try {
        ...
        if (mounted) {
          setAuthError(err.message || "Không lấy được phiên đăng nhập");
        }
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        ...
        if (!data?.hub_id) {
          setAuthError("Tài khoản này chưa được gán chi nhánh trong bảng profiles.");
          setProfile(null);
        } else {
          setProfile(data);
        }
        ...
        if (mounted) {
          setProfile(null);
          setAuthError(err.message || "Không đọc được thông tin chi nhánh");
        }
      } finally {
        ...
        <Card style={{ maxWidth: 520, width: "100%" }}>
          <CardContent style={{ padding: 24 }}>
            ...
            <div style={{ color: "#64748b", marginBottom: 16 }}>
              Kiểm tra lại bảng profiles xem tài khoản này đã có đúng id, hub_id và branch_name chưa.
            </div>
